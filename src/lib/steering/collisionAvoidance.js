import { add, dot, length, subtract, multiply, normalise, } from "../../lib/vector.js";
import { lookWhereYouAreGoing } from "./lookWhereYouAreGoing.js";
export function collisionAvoidance(character, targets, config, alignConfig) {
    const init = {
    ...
    const relativePos = final.firstMinSeparation <= 0 || final.firstDistance < 2 * config.radius
        ...
        : // Otherwise calculate the future relative position
            add(final.firstRelativePos, multiply(final.firstRelativeVel, final.shortestTime));
    // Avoid the target
    return {
