import { length, distance, subtract, multiply, normalise, } from "../../lib/vector.js";
export function arrive(character, target, config) {
    // Config
    ...
    const idealSpeed = distanceToTarget > config.slowRadius
        ? config.maxSpeed
    ...
    const finalLinear = length(linear) > config.maxAcceleration
