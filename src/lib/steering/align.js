...
export function align(character, orientation, config) {
    const linear = [0, 0];
    ...
    const angular = (nextIdealRotation - character.rotation) / config.timeToTarget;
    const angularAcceleration = Math.abs(angular);
    const finalAngular = angularAcceleration > config.maxAngularAcceleration
