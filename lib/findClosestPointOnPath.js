// @flow

import type { Path } from "./drawPath.js";
import type { Vector } from "./vector.js";

type Segment = [Vector, Vector];

const zero2D = [0, 0];

function vectorToSegment2D(
  t: number,
  [px, pz]: Vector,
  [[ax, az], [bx, bz]]: Segment
): Vector {
  return [(1 - t) * ax + t * bx - px, (1 - t) * az + t * bz - pz];
}

function sqDiag([x, z]: Vector) {
  return x ** 2 + z ** 2;
}

function findClosestPointOnSegment(
  [ax, az]: Vector,
  [bx, bz]: Vector,
  [px, pz]: Vector
): Vector {
  const v = [bx - ax, bz - az]; // distance from a to b
  const u = [ax - px, az - pz]; // distance from p to a

  const vu = v[0] * u[0] + v[1] * u[1]; // sum the multiplied distances?
  const vv = sqDiag(v); // dist from a->b squared
  const t = -vu / vv;

  if (t >= 0 && t <= 1) {
    return vectorToSegment2D(t, zero2D, [
      [ax, az],
      [bx, bz],
    ]);
  }

  const g0 = sqDiag(
    vectorToSegment2D(
      0,
      [px, pz],
      [
        [ax, az],
        [bx, bz],
      ]
    )
  );
  const g1 = sqDiag(
    vectorToSegment2D(
      1,
      [px, pz],
      [
        [ax, az],
        [bx, bz],
      ]
    )
  );

  return g0 <= g1 ? [ax, az] : [bx, bz];
}

function distanceToSegment(
  [[ax, az], [bx, bz]]: Segment,
  [px, pz]: Vector // point
): number {
  const A = px - ax;
  const B = pz - az;
  const C = bx - ax;
  const D = bz - az;

  const dot = A * C + B * D;
  const lengthSquared = C ** 2 + D ** 2;
  const param = lengthSquared === 0 ? -1 : dot / lengthSquared;

  if (param < 0) {
    const dx = px - ax;
    const dy = pz - az;
    return Math.sqrt(dx ** 2 + dy ** 2);
  }

  if (param > 1) {
    const dx = px - bx;
    const dy = pz - bz;
    return Math.sqrt(dx ** 2 + dy ** 2);
  }

  const dx = px - ax + param * C;
  const dy = pz - az + param * D;

  return Math.sqrt(dx ** 2 + dy ** 2);
}

function findClosestSegmentToPoint(
  point: Vector,
  path: Path
): [Vector, Vector] {
  const [a, b, ...rest] = path;
  const firstSegment: Segment = [a, b];
  const distToFirst = distanceToSegment(firstSegment, point);

  return rest.reduce(
    (
      acc: { shortestDist: number, segment: Segment },
      vec: Vector,
      index: number
    ) => {
      const prevVec = index === 0 ? b : rest[index - 1];
      const segment = [prevVec, vec];
      const dist = distanceToSegment(segment, point);

      return acc.shortestDist < dist ? acc : { shortestDist: dist, segment };
    },
    {
      shortestDist: distToFirst,
      segment: firstSegment,
    }
  ).segment;
}

export default function findClosestPointOnPath(
  path: Path,
  point: Vector
): Vector {
  if (path.length < 2) {
    return point;
  }
  const [a, b] = findClosestSegmentToPoint(point, path);
  return findClosestPointOnSegment(a, b, point);
}
