...
import { add, distance, subtract, multiply, normalise } from "./vector.js";
...
export function getParam(
  ...
  // from the start to the nearest point on the path to the characterPosition,
  // that is closest to the targetPosition
  // find param for target
  // find all params
  // PARAM for target ----------------
  // starting at node 0,
  //   - find the distance from the node to the target
  //   - find the distance from the node to the next node
  //   - given the target is closer, that is the distance
  //   - given the next node is closer, accumulate and start again from the next node until either
  ...
  //   - find the closest point on the segment to the character
  //   - ???
}
...
export function findClosestSegmentToPoint(
  ...
  const segments = path.reduce((acc, vec, index) => {
